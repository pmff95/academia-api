...
public class EurekaException extends RuntimeException {
    ...
    public static EurekaException mapToEureka(int status, String body) {
        String msg = "Erro " + status + ": " + body;

        return switch (status) {
            case 400 -> EurekaException.ofValidation(msg);
            case 401 -> EurekaException.ofUnauthorized(msg);
            case 403 -> EurekaException.ofForbidden(msg);
            case 404 -> EurekaException.ofNotFound(msg);
            case 409 -> EurekaException.ofConflict(msg);
            case 204 -> EurekaException.ofNoContent(msg);
            default -> EurekaException.ofException(msg);
        };
    }
